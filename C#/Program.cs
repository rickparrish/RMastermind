...
namespace RandM.RMastermind
{
    class Program
    {
        ...
        static string PROG_VER = "RMastermind v12.12.29";
        ...
        static void Main(string[] args)
        {
            ...
            Door.OnHangUp = new Door.OnHangUpCallback(_OnHangUp);
            Door.OnStatusBar = new Door.OnStatusBarCallback(_OnStatusBar);
            Door.OnSysOpKey = new Door.OnSysOpKeyCallback(_OnSysOpKey);
            Door.OnTimeOut = new Door.OnTimeOutCallback(_OnTimeOut);
            Door.OnTimeOutWarning = new Door.OnTimeOutWarningCallback(_OnTimeOutWarning);
            Door.OnTimeUp = new Door.OnTimeUpCallback(_OnTimeUp);
            Door.OnTimeUpWarning = new Door.OnTimeUpWarningCallback(_OnTimeUpWarning);
            Door.Startup(args);
            ...
            do
            {
                ...
                if ((GameOver) && (Ch != 'N') && (Ch != 'Q')) continue;

                switch (Ch)
                {
                    case '2': MoveColour(+1); break;
                    case '4': MovePiece(-1); break;
                    case '6': MovePiece(+1); break;
                    case '8': MoveColour(-1); break;
                    case 'H': HighScores(); RedrawScreen(); break;
                    case 'N': if (GameOver) { NewGame(); } else { ChangeMsg("Finish This Game First"); } break;
                    case 'Q': break; // Must have or "Q is not a valid choice!" will be displayed
                    case ' ': GuessLine(); break;
                    case '\r': PlacePiece(); break;
                    default: ChangeMsg(Ch + " is not a valid choice!"); break;
                }
            } while (Ch != 'Q');
            HighScores();
            Door.Shutdown();
        ...
        static void _OnCLP(object sender, CommandLineParameterEventArgs e)
        {
            ...
            switch (e.Key)
            {
                case 'X': Crt.WriteLn("Parameter -X passed with value of: " + e.Value); break;
            }
        }

        // Event for when the user drops carrier }
        static void _OnHangUp()
        {
        ...
        static void _OnStatusBar()
        {
            switch (Status.Current)
            {
                case 0:
                    ...
                    Crt.FastWrite(StringUtils.PadRight("Idle: " + StringUtils.SecToMS(Door.TimeIdle()), ' ', 11), 51, 25, 31);
        ...
        static bool _OnSysOpKey(char AKey)
        {
            switch (AKey)
            {
                case '\x2E': //ALT-C
                ...
                case '\x3B': //F1
                    ...
                    if (Door.OnStatusBar != null) Door.OnStatusBar();
                    return true;
            }
            return false;
        }
        ...
        static void _OnTimeUpWarning(int AMinutes)
        {
